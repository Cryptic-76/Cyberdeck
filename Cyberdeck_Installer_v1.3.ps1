<#
    Cyberdeck Installer Script v1.3
    Autor: Jörn
    Zweck: Installiert das Cyberdeck PowerShell Modul + nmap + Starship + Arsenal + Profilpatch
#>

Write-Host "=== CYBERDECK INSTALLER v1.3 ===" -ForegroundColor Cyan
Write-Host ""
...
# ------------------------------------------------------------
# 3.5 PowerShellArsenal Installation / Detection
# ------------------------------------------------------------

Write-Host ""
Write-Host "[INFO] Prüfe PowerShellArsenal..." -ForegroundColor Cyan

$moduleName     = "PowerShellArsenal"
$localSource    = "C:\ProgramData\PowerShellArsenal"
$userModulePath = "$HOME\Documents\PowerShell\Modules\$moduleName"
$moduleExists   = Get-Module -ListAvailable -Name $moduleName

# --- FALL 1: Modul ist bereits installiert ---
if ($moduleExists) {
    Write-Host "[OK] PowerShellArsenal ist bereits installiert." -ForegroundColor Green
    try {
        Import-Module $moduleName -Force -ErrorAction Stop
        Write-Host "[OK] Modul erfolgreich geladen." -ForegroundColor Green
    }
    catch {
        Write-Host "[WARNUNG] Modul existiert, konnte aber nicht geladen werden." -ForegroundColor Yellow
    }
}
# --- FALL 2: Modul ist NICHT installiert, aber lokal vorhanden ---
elseif (Test-Path $localSource) {
    Write-Host "[INFO] Lokale PowerShellArsenal-Quelle gefunden: $localSource" -ForegroundColor Cyan

    if (Test-Path $userModulePath) {
        Write-Host "[INFO] Entferne alte Version im Benutzer-Modulpfad..." -ForegroundColor Yellow
        Remove-Item -Recurse -Force $userModulePath
    }

    Write-Host "[INFO] Kopiere PowerShellArsenal nach: $userModulePath" -ForegroundColor Cyan
    Copy-Item -Path $localSource -Destination $userModulePath -Recurse -Force

    try {
        Import-Module $moduleName -Force -ErrorAction Stop
        Write-Host "[OK] PowerShellArsenal erfolgreich installiert & geladen." -ForegroundColor Green
    }
    catch {
        Write-Host "[FEHLER] Konnte PowerShellArsenal nicht importieren." -ForegroundColor Red
    }
}
# --- FALL 3: Weder installiert noch lokal vorhanden ---
else {
    Write-Host "[WARNUNG] PowerShellArsenal wurde nicht gefunden." -ForegroundColor Yellow
    Write-Host "Lege es in C:\ProgramData\PowerShellArsenal ab, um es automatisch zu installieren." -ForegroundColor Yellow
}
...
Set-Alias arsenal Show-ArsenalPanel
